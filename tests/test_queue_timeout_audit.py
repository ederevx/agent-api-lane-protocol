...
class QueueMaintenanceModeTest(_TempGatewayCase):
    """A new interaction discovered while porting queue coalescing
    alongside the independently-developed maintenance-mode feature:
    `handle_queue()` must honor `service.maintenance` with the same
    precedence as `handle()` (§ contract.json `request.queue`), rather
    than silently bypassing it -- otherwise an operator's maintenance
    flag would only ever stop `request.forward` traffic."""

    def test_maintenance_mode_short_circuits_before_provider_lane(
        self,
    ) -> None:
